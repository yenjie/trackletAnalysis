...
int create_hole_map(const char* mc_fname, const char* data_fname, int start, int end) {
    ...
    for (int i=0; i<hratio->GetNcells(); ++i) {
        ...
        if (_XLEDGE(1.400) || _XLEDGE(1.540) || _XLEDGE(1.575) || _XLEDGE(2.065) ||
            ...
            _XLEDGE(2.450)) {
            if (hholes->GetBinContent(i) > 0.4)
                hholes->SetBinContent(i, 0);
            else if (hholes->GetBinContent(i) > 0.2)
                hholes->SetBinContent(i, hholes->GetBinContent(i));
        }
    }
...
#define _ZERO2(low, high) {             \
    ...
    _ZERO(4090); _ZERO(4292); _ZERO(4494); _ZERO(4696); _ZERO(4898);
    _ZERO(12586, 12593);
    _ZERO(28726, 28733);
    ...
    TFile* foutput = new TFile("holes-5tev.root", "recreate");
    ...
    std::ofstream ofs("hholes-5tev.h", std::ofstream::out);
...
int main(int argc, char* argv[]) {
    if (argc == 5)
        return create_hole_map(argv[1], argv[2], atoi(argv[3]), atoi(argv[4]));
    else
        return 1;
}
