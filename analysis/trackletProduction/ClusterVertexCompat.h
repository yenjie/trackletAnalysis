...
Double_t getClusVtxCompat(const std::vector<RecoHit> &hits) {
  ...
  for(Double_t z0 = minZ_; z0 <= maxZ_; z0 += zStep_) {
    nhits = getContainedHits(hits, z0, diff);
    if(nhits == 0)
      continue;
    if(nhits > nhits_max) {
      diff_max = 1e+9;
  ...
  nbest = getContainedHits(hits,zest,diff);
  nminus = getContainedHits(hits,zest-10.,diff);
  nplus = getContainedHits(hits,zest+10.,diff);
