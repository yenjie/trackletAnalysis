...
double angularRangeReduce(double x) {
  ...
  if (x >= -cody_waite_x_max && x <= cody_waite_x_max) {
    const double inverse_two_pi = 0.15915494309189534197;
    const double k = rint(x * inverse_two_pi);
    ret = ((x - (k * two_pi_0)) - k * two_pi_1) - k * two_pi_2;
  }
...
double deltaR2(double eta1, double phi1, double eta2, double phi2) {
  ...
  double dphi = angularRangeReduce(phi1 - phi2);
...
void match_gen_particles(const char* infname, const char* outfname) {
