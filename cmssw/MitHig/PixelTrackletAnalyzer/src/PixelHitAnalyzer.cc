...
class PixelHitAnalyzer : public edm::EDAnalyzer {
   ...
   edm::EDGetTokenT<TrackingVertexCollection> tpVtxSrc_;
   edm::EDGetTokenT<TrackingParticleCollection> trackingParticlesSrc_;
   edm::EDGetTokenT<SimVertexContainer> simVertexSrc_;

   edm::EDGetTokenT<HepMCProduct> genSrc_;
...
PixelHitAnalyzer::PixelHitAnalyzer(const edm::ParameterSet& iConfig)
{
   ...
   if (doMC_ == false) {
      doTrackingParticle_ = false;
   } else {
      genSrc_ = consumes<HepMCProduct> (edm::InputTag("generatorSmeared"));
      tpVtxSrc_ = consumes<TrackingVertexCollection> (iConfig.getUntrackedParameter<edm::InputTag>("tpVtxSrc", edm::InputTag("mergedtruth", "MergedTrackTruth")));
      trackingParticlesSrc_ = consumes<TrackingParticleCollection> (edm::InputTag("mergedtruth", "MergedTrackTruth"));
      simVertexSrc_ = consumes<SimVertexContainer> (edm::InputTag("g4SimHits"));
   }
...
PixelHitAnalyzer::fillVertices(const edm::Event& iEvent) {
   ...
   if (doMC_) {
      ...
      if (doTrackingParticle_) {
         ...
         iEvent.getByToken(tpVtxSrc_, vertices);

         for (unsigned int i=0; i<vertices->size(); ++i) {
            daughter = (*vertices)[i].nDaughterTracks();
            if (daughter >(*vertices)[greatestvtx].nDaughterTracks() && fabs((*vertices)[i].position().z())<30)
               greatestvtx = i;
         }

         if (vertices->size()>0 && fabs((*vertices)[greatestvtx].position().z())<30) {
            pev_.vx[0] = (*vertices)[greatestvtx].position().x();
            pev_.vy[0] = (*vertices)[greatestvtx].position().y();
            pev_.vz[0] = (*vertices)[greatestvtx].position().z();
         } else {
            pev_.vx[0] =  -99;
            pev_.vy[0] =  -99;
            pev_.vz[0] =  -99;
         ...
         if (simvertices->size()>0) {
            if (fabs((*simvertices)[0].position().z())<30) {
               pev_.vx[0]=(*simvertices)[0].position().x();
...
PixelHitAnalyzer::fillHits(const edm::Event& iEvent) {
   ...
   if (doMC_ && doTrackingParticle_)
      iEvent.getByToken(trackingParticlesSrc_, trackingParticles);
   ...
   for (SiPixelRecHitCollection::const_iterator it = rechits->begin(); it != rechits->end(); it++) {
      ...
      unsigned int layer = pdetIdB.layer();
      unsigned int disk = pdetIdF.disk();
      ...
      for (SiPixelRecHitCollection::DetSet::const_iterator recHitIterator = recHitRange.begin(); recHitIterator != recHitRange.end(); ++recHitIterator) {
         ...
         */
         ...
         if (layer == 1 && subid==1) {
            ...
            pev_.cs1[pev_.nhits1] = recHit->cluster()->size();
            pev_.ch1[pev_.nhits1] = recHit->cluster()->charge();
         ...
         if (layer == 2 && subid==1) {
            ...
            pev_.cs2[pev_.nhits2] = recHit->cluster()->size();
            pev_.ch2[pev_.nhits2] = recHit->cluster()->charge();
         ...
         if (layer == 3 && subid==1) {
            ...
            pev_.cs3[pev_.nhits3] = recHit->cluster()->size();
            pev_.ch3[pev_.nhits3] = recHit->cluster()->charge();
         ...
         if (disk == 1 && subid==2) {
            ...
            pev_.cs4[pev_.nhits4] = recHit->cluster()->size();
            pev_.ch4[pev_.nhits4] = recHit->cluster()->charge();
         ...
         if (disk == 2 && subid==2) {
            ...
            pev_.cs5[pev_.nhits5] = recHit->cluster()->size();
            pev_.ch5[pev_.nhits5] = recHit->cluster()->charge();
...
PixelHitAnalyzer::endJob() {
   pixelTree_->Write("", TObject::kOverwrite);
}
